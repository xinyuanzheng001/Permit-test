...
function first(e) {
  e.preventDefault()
  if (confirm('You have 25 minutes, Are you sure want to start？') == true) {
    ...
    let x = setInterval(function () {
      ...
      document.querySelector(
        '#time'
      ).innerHTML = `Time: ${minutes} : ${seconds}`
      if (distance < 0) {
        confirm('时间到')
        clearInterval(x)
        showResult(e)
...
function next(e) {
  ...
  if (t) {
    ...
    if (new_question.childElementCount == 3) {
      ...
      if (!question) {
        question =
          new_question.getElementsByTagName('img')[0].attributes[0].nodeValue
...
function showResult(e) {
  ...
  if (correct_count >= 18) {
    document.querySelector(
      '.result'
    ).innerHTML = `恭喜，你通过了测试，你总共答对了${correct_count}题!`
  } else {
    document.querySelector(
      '.result'
    ).innerHTML = `很遗憾，你没有通过测试, 你答对了${correct_count}题`
  }
}
